...
- **MAJOR** (`X.0.0`): Breaking changes that require users to take action (e.g., changed API or architectural assumptions).
...
Omit categories that have no entries for a given release. Each entry is a bullet point (`-`) with a concise, user-facing description. Check the `docs/` folder for any technical specifics.
...
[unreleased]: https://github.com/josecanciani/fusewire/compare/X.Y.Z...main
[X.Y.Z]: https://github.com/josecanciani/fusewire/compare/X.Y.Z-1...X.Y.Z
[X.Y.Z-1]: https://github.com/josecanciani/fusewire/releases/tag/X.Y.Z-1
```
...
2. **Later, cut a release** (when the user asks):
   ...
   # Move [Unreleased] entries under ## [1.1.0] - 2026-04-25
   npm run changelog
   git add package.json CHANGELOG.md
